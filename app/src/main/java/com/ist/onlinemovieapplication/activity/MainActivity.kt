...
import com.ist.onlinemovieapplication.fragment.AnalogFragment
import com.ist.onlinemovieapplication.fragment.DigitalFragment
import com.ist.onlinemovieapplication.fragment.ProfielFragment
import com.ist.onlinemovieapplication.activity.MainActivity.ViewPagerAdapter
...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupViewPager(viewPager: ViewPager) {
        if (fragsList.size <= 0) {
            fragsList.clear()
            fragsList.add(AnalogFragment())
            fragsList.add(DigitalFragment())
            fragsList.add(ProfielFragment())
        }
        if (tittle.size <= 0) {
            tittle.clear()
            tittle.add("Analog")
    ...
    private fun setupLocationListeners() {
        if (listeners.size <= 0) {
            listeners.add(fragsList[0] as locationCallback?)
            listeners.add(fragsList[1] as locationCallback?)
        }
    }
    ...
    private fun setListeners(){
        ...
        binding.mapBtn.setOnClickListener {
            ...
            locationForGfResult.launch(Intent(this,MapActivity::class.java))
